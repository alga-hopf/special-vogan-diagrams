...
# This file was *autogenerated* from the file /home/alice/special-vogan-diagrams/script/specialVoganDiagramSingle.sage
...
if __name__ == "__main__":
	...
	if all(phi0[k]==_sage_const_0  for k in range(len(phi0))):	# If phi0 is 0, then the orbit is symplectic Calabi-Yau
		...
		print(phi0*(_sage_const_1 /gcd(phi0)),'   ','Non-compact simple roots:',P,'   ','S:',S,'   ','symplectic Fano')
		Omega=[root for root in Set(positiveRoots).difference(Set([root for root in positiveRoots if all(root[k]==_sage_const_0  for k in P)]))]
		print("Dimension V:",(_sage_const_2 *(len(positiveRoots)-len(Omega))+rank),"    Dimension G/V:",_sage_const_2 *len(Omega))
		print("Hermitian scalar curvature:",-_sage_const_4 *gcd(phi0)*sum(sum(epsilon[alpha]*((alpha*B*beta)/(sum(phi0[z]*beta[z]*B[z,z] for z in range(rank)))) for alpha in Omega) for beta in Omega))
		isSpecial=_sage_const_0 
	else:
